...
impl ModelBankOwner
{
    ...
    {
        ModelBankOwner{ model_banks: (0..number_render_systems).into_iter().map(|_| ModelBank::new()).collect(), number_models_loaded: 0, free_ids: Vec::new() }
    }
    ...
    fn upload_model_geometry<A: AsRef<Path> + Debug + Clone>(&mut self, location: A, render_system_index: u32, model_id: ModelId, render_flow: &mut RenderFlow)
    {
        let (mut models, materials) = tobj::load_obj(location, true).unwrap();
    ...
    pub fn register_upload_generated_models(&mut self, render_system_index: RenderSystemIndex, geometry: Vec<ModelGeometry>) -> ModelId
    {
        let base_model_id = self.get_model_id(render_system_index);

        for (index, level_of_view_geometry) in geometry.into_iter().enumerate()
        {
            let mut model_aabb = StaticAABB::point_aabb();
            ...
            for mesh in &level_of_view_geometry.meshes
            {
                // Combine all of the mesh AABB to find the overall bounding volume of the model
                model_aabb = model_aabb.combine_aabb(&aabb_helper_functions::calculate_aabb(&mesh.vertices));
            }

            self.model_banks[render_system_index.index as usize].add_model(adjusted_model_id, level_of_view_geometry, model_aabb);
        }
    ...
    pub fn register_model<T: Into<String>>(&mut self, model_info: &LoadModelInfo<T>, render_flow: &mut RenderFlow) -> ModelId
    {
        ...
        for x in 0..model_info.location.len()
        {
            ...
            self.upload_model_geometry(model_info.location[x].clone(), model_info.render_system_index.index as u32, adjusted_model_id, render_flow);
        }
