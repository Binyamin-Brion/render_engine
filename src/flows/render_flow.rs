...
use nalgebra_glm::{TMat4, TVec3, vec4};
...
{
    pub visible_world_sections: Vec<UniqueWorldSectionId>,
...
{
    visible_world_sections: &'a Vec<UniqueWorldSectionId>,
    ecs: &'a ECS,
...
impl RenderFlow
{
    ...
    pub fn new(mut render_systems: Vec<RenderSystem>, level_of_views: Vec<LevelOfView>, window_dimensions: (i32, i32), shadow_draw_fn: DrawFunction) -> RenderFlow
    {
        // Only one result after uploading models into a render system
        let (tx, rx) = sync_channel(1);
        render_systems.push(RenderFlow::create_shadow_render_system(level_of_views, shadow_draw_fn));
    ...
    fn run_render_system(&mut self, upload_models: Option<Range<usize>>, render_system_index: usize, render_args: &RenderArguments, visible_sections_light: &HashSet::<UniqueWorldSectionId>)
    {
    ...
    fn extract_static_data(sorting_param: &SortWorldSectionEntitiesParam, static_data: Arc<RwLock<Vec<UniqueSectionData>>>, render_system_index: usize) -> Arc<Mutex<SortResult>>
    {
        ...
        if sorting_param.visible_world_sections.is_empty()
        {
        ...
        {
            let chunk_size = sorting_param.visible_world_sections.len();

            let _ = sorting_param.visible_world_sections.chunks(chunk_size)
                .map(|x|
                {
                    extract_fn(x);
                }).collect::<()>();
        ...
        else
        {
            ...
            let _ = sorting_param.visible_world_sections.par_chunks(chunk_size).map(|x|
                {
                    extract_fn(x);
                }).collect::<()>();
        };
    ...
    fn sort_world_section_active_entities(sorting_param: SortWorldSectionEntitiesParam) -> Arc<Mutex<HashMap<ModelId, HashMap<SortableIndex, WrittenInformation>>>>
    {
        ...
        for x in sorting_param.visible_world_sections
        {
            ...
            for (sortable_component_index, data) in layout_info
            {
                for (layout_index, layout_data) in data.layout_data.iter()
                {
                    ...
                    buffer_bytes_written[buffer_index] +=
                        MappedBuffer::write_data_serialized(mapped_instance_buffers[buffer_index], layout_data, buffer_bytes_written[buffer_index]);
                }
        ...
        for model_bank in model_banks
        {
            for (model_id, model_information) in model_bank.stored_models()
            {
                ...
                for mesh in &model_information.geometry.meshes
                {
                    ...
                    MappedBuffer::write_data_serialized(indice_buffer, &mesh.indices, (number_indices_uploaded * size_of::<u32>()) as isize);
    ...
    fn create_shadow_render_system(level_of_views: Vec<LevelOfView>, shadow_draw_fn: DrawFunction) -> RenderSystem
    {
         RenderSystemBuilder::new()
            ...
            .with_draw_function(shadow_draw_fn)
