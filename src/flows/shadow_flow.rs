...
{
    NoNewMapRequired,
    NewMapRequired(Camera, Vec<UniqueWorldSectionId>, TextureArrayIndex)
}
...
impl ShadowFlow
{
            ...
            Some(entity_id) =>
                {
                    ...
                    let position = args.ecs.get_copy::<Position>(entity_id).unwrap().0;
                    ...
                    let visible_world_sections =
                        VisibleWorldFlow::find_visible_world_ids(Arc::new(render_frustum_culler), light_camera.get_position(), light_camera.get_far_draw_distance(), args.tree);
    ...
    fn find_next_light_to_have_shadow_map(&mut self, args: &CalculationArgs, light_type: FindLightType) -> Option<EntityId>
    {
        ...
        let nearby_light_sources = find_nearby_lights(args.visible_sections_light, args.tree, light_type);
...
/// `bounding_box_tree` - structure that divides the world into sub-sections
pub fn find_nearby_world_sections_maps(camera_pos: TVec3<f32>, radius: f32, bounding_box_tree: &BoundingBoxTree) -> HashSet::<UniqueWorldSectionId>
{
    ...
    VisibleWorldFlow::find_visible_world_ids_map(Arc::new(culler), camera_pos, radius, bounding_box_tree)
}