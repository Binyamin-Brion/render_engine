...
use crate::exports::load_models::InstanceLogic;
use crate::exports::logic_components::{CanCauseCollisions, IsOutOfBounds, ParentEntity, RenderSystemIndex};
...
{
    pub visible_world_sections: HashSet::<UniqueWorldSectionId>,
...
impl LogicFlow
{
    ...
    pub fn new(instance_logic: InstanceLogic) -> LogicFlow
    {
        let mut ecs = ECS::new();
    ...
    pub fn execute_logic(&mut self, args: ExecutionArgs, render_flow: &mut RenderFlow) -> Vec<FrameChange>
    {
        ...
        for x in &args.visible_world_sections
        {
        ...
        for entity in entities
        {
            ...
            if logic_flow.ecs.check_component_written_assume_registered::<Velocity>(*entity)
            {
                ...
                if logic_flow.ecs.check_component_written_assume_registered::<Acceleration>(*entity)
                {
                    ...
                    let change_velocity = acceleration * elapsed_time;
                    if nalgebra_glm::length(&change_velocity.0) != 0.0
                    {
                ...
                let translation = velocity * elapsed_time;
                if nalgebra_glm::length(&translation.0) != 0.0
                {
                ...
                {
                    logic_flow.expected_frame_changes.lock().push(FrameChange::EntityChange(EntityChangeInformation::ModifyRequest(entity_change_request)));
                }
            ...
            if logic_flow.ecs.check_component_written_assume_registered::<VelocityRotation>(*entity)
            {
                ...
                if logic_flow.ecs.check_component_written_assume_registered::<AccelerationRotation>(*entity)
                {
                    ...
                    let change_velocity_rotation = acceleration_rotation * elapsed_time;
                    if nalgebra_glm::length(&change_velocity_rotation.0) != 0.0
                    {
                ...
                let change_rotation = velocity_rotation * elapsed_time;
                if nalgebra_glm::length(&change_rotation.0) != 0.0
                {
                ...
                WorldSectionLookup::Unique(i) =>
                    {
                        ...
                        {
                            Some(entities) => entities.push(*entity),
                            None => { relevant_world_sections.insert(*i, Vec::new()); },
                        }
                    }
            }
        }
        ...
        let apply_collision_only_to_self = |this_entity: EntityId, other_entity: EntityId|
            {
                if let Some(this_entity_type) = self.ecs.get_entity_type(this_entity)
                {
                    ...
                    {
                        if let Some(change_request) = (collision_logic.logic)(this_entity, EntityIdRead::new(other_entity), &self.ecs, args.bounding_box_tree)
                        {
                            self.expected_frame_changes.lock().push(FrameChange::EntityChange(change_request));
                        }
                    }
                }
            };
        ...
        let apply_entity_logic = |ecs: &ECS, entities: &HashSet::<EntityId>, elapsed_time: f32|
        {
            for entity in entities
            {
                if let Some(entity_type) = self.ecs.get_entity_type(*entity)
                {
                    ...
                    {
                        if let Some(change_request) = (entity_logic.logic)(*entity, ecs, args.bounding_box_tree, elapsed_time)
                        {
                            self.expected_frame_changes.lock().push(FrameChange::EntityChange(change_request));
                        }
                    }
                    ...
                    if let Some(entity_logic) = self.instance_logic.random_entity_logic.get(&entity_type)
                    {
                        if let Some(change_request) = (entity_logic.logic)(*entity, ecs, args.bounding_box_tree, elapsed_time)
                        {
                            self.random_frame_changes.lock().push(FrameChange::EntityChange(change_request));
                        }
                    }
                }
            }
    ...
    pub fn update_bounding_box_tree(&mut self, bounding_box_tree: &mut BoundingBoxTree, model_bank_owner: Arc<RwLock<ModelBankOwner>>, camera: &mut Camera, render_flow: &mut RenderFlow)
    {
        ...
        let mut changes = &mut *self.random_frame_changes.lock();
