...
impl LogicFrustumCuller
{
    ...
    ///
    /// `render_distance` - the far draw distance of the camera
    /// `camera_position` - the position of the camera
    pub fn new(render_distance: f32, camera_position: TVec3<f32>) -> LogicFrustumCuller
    {
        LogicFrustumCuller{ render_distance, camera_position }
    }
}
...
impl TraversalDecider for LogicFrustumCuller
{
    ...
    fn aabb_in_view(&self, aabb: &StaticAABB) -> bool
    {
        ...
        distance_to_closest_point <= (self.render_distance * 0.25).max(50.0)
    }
}