...
impl EntityTransformationBuilder
{
    ...
    pub fn apply_choices(&mut self, mut original_aabb: StaticAABB, ecs: &mut ECS, bounding_tree: &mut BoundingBoxTree)
    {
        ...
        ecs.write_component::<TransformationMatrix>(self.entity_id, TransformationMatrix(transformation_matrix));
        ...
        {
            eprintln!("Position {:?} is an invalid location", self.translation.unwrap().0);
        }
    }
    ...
    fn write_components(&mut self, ecs: &mut ECS) -> nalgebra_glm::Mat4x4
    {
        ...
        if let Some(translation) = self.translation
        {
            ...
            transformation_matrix = nalgebra_glm::translate(&transformation_matrix, &translation.0);
        }
