...
use crate::window::movement_keys::{KeyID, KeyStatus, MovementKeys};
...
impl Camera
{
    ...
    /// Moves the camera in the direction of the keys that are pressed down. Returns the amount of time
    /// that has passed since the last time this function was called
    pub fn use_wasd_keys(&mut self, keys: &MovementKeys, delta_time: f32) -> f32
    {
        if keys.keys[KeyID::KeyA as usize] == KeyStatus::Pressed
        {
            self.position -= normalize(&cross(&self.direction, &self.up)) * self.movement_speed_factor * delta_time;
        }

        if keys.keys[KeyID::KeyD as usize] == KeyStatus::Pressed
        {
            self.position += normalize(&cross(&self.direction, &self.up)) * self.movement_speed_factor * delta_time;
        }

        if keys.keys[KeyID::KeyW as usize] == KeyStatus::Pressed
        {
            self.position += &self.direction * self.movement_speed_factor * delta_time;
        }

        if keys.keys[KeyID::KeyS as usize] == KeyStatus::Pressed
        {
            self.position -= &self.direction * self.movement_speed_factor * delta_time;
        }
