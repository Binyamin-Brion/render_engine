...
const MIDDLE_BUTTON: MouseButton = MouseButton::Button3;
...
pub struct GLWindow
{
    ...
    key_status: Vec<(Key, Action)>,
    cursor_button_status: Vec<(MouseButton, Action)>,
...
impl GLWindowBuilder
{
    ...
    pub fn build(&self) -> Result<GLWindow, GLFWindowCreationError>
    {
        ...
        let window = GLWindow
        {
            glfw, window, events, wasd_keys: MovementKeys::new(), key_status: Vec::new(),
            cursor_button_status: Vec::new(), latest_cursor_pos: None, middle_button_down: false,
...
impl GLWindow
{
    ...
    /// Checks if any keys match the given function
    ///
    /// `function` - the function that checks whether a certain key has been recorded with a specific action
    #[allow(dead_code)]
    pub fn check_keys<F: Fn((Key, Action)) -> bool>(&self, function: F) -> bool
    {
        self.key_status.iter().find(|x| function(**x)).is_some()
    }
    ...
    /// Get all of the keys that have been acted upon
    #[allow(dead_code)]
    pub fn get_keys_modified(&self) -> &Vec<(Key, Action)>
    {
        &self.key_status
    }

    /// Get all of the mouse buttons that have been acted upon
    #[allow(dead_code)]
    pub fn get_cursor_button_status(&self) -> &Vec<(MouseButton, Action)>
    {
        &self.cursor_button_status
    }
    ...
    /// Check if any of the cursor button acted upon match the given function query
    ///
    /// `function` - function that checks for a specific combination of cursor button and action
    #[allow(dead_code)]
    pub fn check_cursor_buttons<F: Fn((MouseButton, Action)) -> bool>(&self, function: F) -> bool
    {
        self.cursor_button_status.iter().find(|x| function(**x)).is_some()
    }
                ...
                glfw::WindowEvent::Key(key, _, action, _) =>
                    {
                        ...
                        self.key_status.push((key, action));
                    }
                glfw::WindowEvent::MouseButton(button, action, _) =>
                    {
                        self.input_history.update_mouse_members(button, action);
                        self.cursor_button_status.push((button, action));
                    },
                glfw::WindowEvent::CursorPos(x, y) =>
                    {
                        self.latest_cursor_pos = Some((x as i32, y as i32));
                    },
                glfw::WindowEvent::Size(width, height) =>
                    {
    ...
    fn clear_input_history(&mut self)
    {
        ...
        self.cursor_button_status.clear();
