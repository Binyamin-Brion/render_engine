...
impl BoundingBoxTree
{
    ...
    fn normalize_aabb(aabb: &mut StaticAABB, tree_outline_length: f32) -> bool
    {
        ...
        aabb.x_range.min.max(0.0).min(tree_outline_length);
        aabb.y_range.min.max(0.0).min(tree_outline_length);
        aabb.z_range.min.max(0.0).min(tree_outline_length);

        aabb.x_range.max.max(0.0).min(tree_outline_length);
        aabb.y_range.max.max(0.0).min(tree_outline_length);
        aabb.z_range.max.max(0.0).min(tree_outline_length);
