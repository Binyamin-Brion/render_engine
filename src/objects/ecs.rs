...
/// Converts the type ids to the serializable version
///
/// `input` - the type ids to convert to the serializable version
fn convert_typeid(input: &[TypeId]) -> Vec<TypeIdentifier>
{
    let mut casted_components = Vec::with_capacity(input.len());
    for x in input
    {
        casted_components.push(TypeIdentifier::from(*x));
    }
    casted_components
}
