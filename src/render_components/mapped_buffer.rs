...
impl MappedBuffer
{
    ...
    pub fn write_data_serialized<T: 'static>(write_information: BufferWriteInfo, data: &[T], offset_count: isize) -> isize
    {
        let size_type = size_of::<T>();
        let bytes_to_write = data.len() * size_type;

        if bytes_to_write > write_information.size_buffer_bytes as usize
        {
            eprintln!("Attempting to write {} bytes of data into buffer of {} bytes large", bytes_to_write, write_information.size_buffer_bytes);
        }
    ...
    /// `data` - the data to write to the buffer
    /// `offset_bytes` - offset in bytes from the start of the buffer to write the provided data to
    pub fn write_single_serialized_value<T: 'static>(write_information: BufferWriteInfo, data: T, offset_count: isize) -> isize
    {
        MappedBuffer::write_data_serialized(write_information, &[data], offset_count)
    }
