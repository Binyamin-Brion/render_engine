...
impl RenderSystem
{
    ...
    pub fn new(first_render_pass_resources: RenderPassResources, second_render_pass_resources: Option<RenderPassResources>,
               ...
               draw_fn_accessible_fbo: HashMap<String, FBO>,
               upload_local_lights: bool,
               max_light_constraints: MaxNumLights) -> RenderSystem
    {
        RenderSystem
        {
...

        {
            ...
            if let Some(ref mut second_pass_render) = self.second_render_pass_resources
            {
                ...
                if self.upload_local_lights
                {
                    ...
                    any_light_source_visible |= RenderSystem::upload_directional_lights(in_draw_param.visible_sections_light, &mut second_render_pass_draw_param, in_draw_param.visible_directional_lights, self.max_num_lights.directional);
                    any_light_source_visible |= RenderSystem::upload_point_lights(in_draw_param.visible_sections_light, &mut second_render_pass_draw_param, in_draw_param.visible_point_lights,self.max_num_lights.point);
                    any_light_source_visible |= RenderSystem::upload_spot_lights(in_draw_param.visible_sections_light, &mut second_render_pass_draw_param, in_draw_param.visible_spot_lights, self.max_num_lights.spot);
    ...
    fn upload_directional_lights(visible_world_sections: &HashSet<UniqueWorldSectionId>, draw_param: &mut DrawParam,
                                 directional_lights: &mut HashSet::<EntityId>, max_direction_lights: u16) -> AnyLightSourceVisible
    {
        ...
        let mut light_upload_information = LightUploadInformation::new(number_rendered_directional_lights);

        for (index, directional_light) in visible_directional_lights.iter().take(number_rendered_directional_lights).enumerate()
        {
    ...
    fn upload_spot_lights(visible_world_sections: &HashSet<UniqueWorldSectionId>, draw_param: &mut DrawParam,
                          spot_lights: &mut HashSet::<EntityId>, max_spot_lights: u16) -> AnyLightSourceVisible
    {
        ...
        let mut light_upload_information = LightUploadInformation::new(number_rendered_spot_lights);

        for (index, spot_light) in visible_spot_lights.iter().take(number_rendered_spot_lights).enumerate()
        {
            ...
            light_upload_information.positions[index] = position.0;
            light_upload_information.diffuse_colours[index] = light_info.diffuse_colour;
            light_upload_information.specular_colours[index] = light_info.specular_colour;
            light_upload_information.ambient_colours[index] = light_info.ambient_colour;
        ...
        draw_param.write_uniform_value("spotLightPositions", light_upload_information.positions);
        ...
        draw_param.write_uniform_value("numberSpotLights", vec![number_rendered_spot_lights as u32]);
    ...
    fn upload_point_lights(visible_world_sections: &HashSet::<UniqueWorldSectionId>, draw_param: &mut DrawParam,
                           point_lights: &mut HashSet::<EntityId>, max_point_lights: u16)  -> AnyLightSourceVisible
    {
        let max_number_point_lights = 1;
        ...
        let number_rendered_spot_lights = visible_point_lights.len().min(max_point_lights as usize);
        let mut light_upload_information = LightUploadInformation::new(number_rendered_spot_lights);

        for (index, point_light) in visible_point_lights.iter().take(number_rendered_spot_lights).enumerate()
        {
        ...
        draw_param.write_uniform_value("pointLightPositions", light_upload_information.positions);
        draw_param.write_uniform_value("pointLightDirections", light_upload_information.directions);
        ...
        draw_param.write_uniform_value("numberPointLights", vec![number_rendered_spot_lights as u32]);
