...
use crate::{ArrayIndexer, ChangeHistory, EXIT_GRACEFULLY_COUNT, FAILURE_COUNT, FrameVectors,
            get_debug_logs_folder, handle_cursor_movement, handle_key_input, handle_wasd_movement,
            HISTORY_THREAD_SUCCESS_COUNT, LoadParam, RENDER_THREAD_ID, RENDER_THREAD_SUCCESS_COUNT,
            StoredHistoryState, UserUploadInformation};
...
use crate::threads::private_common_structurtes::{CAMERA, DELTA_TIME};
...
pub fn render_world(mut args: RenderInputArgs, user_load_info: UserUploadInformation, debug_mode: Option<LoadParam>)
{
    ...
    let mut window = GLWindowBuilder::new()
    ...
    *CAMERA.write() = user_load_info.initial_camera;
            ...
            RenderSystemType::Default(i) =>
                {
                    create_default_render_system
                        (
                            i.draw_function, i.instance_layout_update_fn, i.level_of_views, i.window_resolution, i.sky_boxes, i.max_count_lights,
                        )
                }
    ...
    {
        let (temp_pipeline, camera) = Pipeline::new_from_file(load_param.clone(),
                                                              render_systems, shadow_lov, window.window.get_size(),
                                                              user_load_info.shadow_draw_fn, user_load_info.instance_logic);
    ...
    else
    {
        ...
        render_pipeline = Pipeline::new(render_systems, (16_384, user_load_info.world_section_length), user_load_info.instance_logic,
...
fn handle_user_input(window: &mut GLWindow, current_mode: &mut CurrentMode, play: &mut bool)
{
    ...
    match current_mode
    {
        CurrentMode::Debug  =>
            {
                handle_key_input!(window, Escape, Press, *current_mode = CurrentMode::DCustomMovement);
                handle_key_input!(window, Right, Press, *play = true);
                handle_key_input!(window, Right, Release, *play = false);
            },
        CurrentMode::OnePastLastFrame =>
            {
                handle_key_input!(window, Escape, Press, { window.set_window_close(); *RENDER_THREAD_SUCCESS_COUNT.lock() = EXIT_GRACEFULLY_COUNT; });
                handle_key_input!(window, Up, Press, *play = true);
            },
        CurrentMode::OnePastLaseFramePause =>
            {
                handle_key_input!(window, Escape, Press, { window.set_window_close(); *RENDER_THREAD_SUCCESS_COUNT.lock() = EXIT_GRACEFULLY_COUNT; });
                handle_key_input!(window, Right, Press, *current_mode = CurrentMode::Run);
            },
        CurrentMode::DCustomMovement =>
            {
                handle_key_input!(window, Escape, Press, { window.set_window_close(); *RENDER_THREAD_SUCCESS_COUNT.lock() = EXIT_GRACEFULLY_COUNT; });
                handle_key_input!(window, Insert, Press, *current_mode = CurrentMode::Debug);
                handle_key_input!(window, Right, Press, *play = true);
                handle_key_input!(window, Right, Release, *play = false);
                handle_wasd_movement!(window, CAMERA, delta_time);
                handle_cursor_movement!(window, CAMERA);
            }
        CurrentMode::Run =>
            {
                handle_key_input!(window, Escape, Press, { window.set_window_close(); *RENDER_THREAD_SUCCESS_COUNT.lock() = EXIT_GRACEFULLY_COUNT; });
    ...
    unsafe
        {
            ...
            match current_mode
            {
                CurrentMode::Debug | CurrentMode::DCustomMovement =>
                    {
                        ...
                        if render_pipeline.debug_execute(*current_mode == CurrentMode::DCustomMovement, CAMERA.clone(), *play, &window.input_history)
                        {
                ...
                CurrentMode::OnePastLastFrame =>
                    {
                        ...
                        if *play
                        {
                            ...
                            render_pipeline.execute(CAMERA.clone(), *DELTA_TIME.read(), &window.input_history);
                        ...
                        else
                        {
                            ...
                            render_pipeline.debug_execute(*current_mode == CurrentMode::DCustomMovement, CAMERA.clone(), false, &window.input_history);
                        }
                    },
                CurrentMode::OnePastLaseFramePause =>
                    {
                ...
                CurrentMode::Run =>
                    {
                        let mut changes = render_pipeline.execute(CAMERA.clone(), *DELTA_TIME.read(), &window.input_history);
